describe('Authentication Flow Tests', () => {
  ...
  it('Test Case 2: Login User with correct email and password', () => {
    ...
    // Upon successful login we're redirected to main page
  ...
  it('Test Case 6: Verify All Products and product detail page', () => {
    // Visit the page containing the product grid
    cy.visit('https://localhost:57131/');

    // Verify that the product grid exists and contains at least one product
    ...
    // Extract the name of the first product to verify it on the details page
    cy.get('.product-card').first().find('.product-name').invoke('text').then((productName) => {
      ...
      // Verify other essential elements on the product detail page
  ...
  it('Test Case 7: Search Product', () => {
    // 1. Visit the page where the search bar is located
    cy.visit('https://localhost:57131/products');

    const searchTerm = 'Dino';

    // Find the search input field and type the search term
    ...
    // 5. Verify that the displayed products actually match the search term
    ...
    // Clear the search bar and type a gibberish term
  ...
  it('Test Case 8: Add Products in Cart and Verify Cart States', () => {
    ...
    // Retrieve the safely trimmed string
  ...
  it('Test Case 9: Verify Product quantity in Cart and Subtotal calculation', () => {
    ...
    cy.get('.product-card').first().within(() => {
      // Safely extract and store the product name
  ...
  it('Test Case 11: Place Order: Login before Checkout', () => {
    ...
    // We assume this user already exists in your database and has an address saved
    const existingEmail = 'Sorin@gmail.com';
    ...
    // Return to the homepage products list
    cy.visit('https://localhost:57131/');
    ...
    // Ensure the item is in cart then proceed to checkout
    cy.get('.shop-table').should('be.visible');
    cy.contains('button', 'Proceed to Checkout').click();

    // Verify that it is on checkoutpage
    cy.url().should('include', '/checkout');
  ...
  it('Test Case 12: Remove Products From Cart', () => {
    ...
    //  Click the "Remove" button inside the FIRST row of the table
    cy.get('.shop-table tbody tr').first().within(() => {
      cy.get('.btn-remove').click();
    });

    // Verify the table updated and now only has 1 row left
